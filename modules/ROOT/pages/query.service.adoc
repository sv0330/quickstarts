...
The CSV format does not contain response metadata but only the query results. The response contains a line for each row, where each line contains the row columns separated by a comma. The following example returns the data as comma-separated values.
...
* For API reference: 
** link:https://downloads.teradata.com/api/teradata_query_service[API Reference Guide]
** On link:https://docs.teradata.com/[Teradata Documentation Portal], you can download the following resources from the attachment in the left sidebar:
